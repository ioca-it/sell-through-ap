...
export const METRIC_DEFINITIONS = Object.freeze([
  ...
  defineMetric('eolDiscountApplicable', 'SKU EOL que aplica regla de descuento', 'Subconjunto operativo de SKU EOL que cumplen la regla de descuento vigente.', 'Estado = EOL e Inventario Final ≥ EOL_DISCOUNT_MIN_INVENTORY (12 unidades) y Descuento consumidor > 0%.', 'SKU', 'Product Master + Inventario del Cliente + EOL Engine', 'No modifica el KPI EOL general (SKU EOL); un SKU EOL por debajo del umbral permanece clasificado como EOL pero no entra en esta tabla operativa.'),
  defineMetric('eolDiscountMinInventory', 'EOL — Inventario mínimo para aplicar descuento', 'Cantidad mínima de Inventario Final requerida para que un SKU EOL pueda entrar en el subconjunto operativo de descuento.', 'Inventario Final ≥ 12 AND Descuento vigente > 0%.', 'unidades', 'Regla de negocio EOL', 'Default vigente 12; los SKU EOL por debajo del umbral permanecen clasificados como EOL pero no forman parte de esta tabla. Candidato futuro a Configuration Center; no editable todavía.'),
  ...
  defineMetric('tierInventoryValue', 'Valor Inventario Total', 'Valor monetario del mismo universo Inventario Actual del Cliente (Inventario Final > 0) que alimenta el bloque Tier de Inventario.', 'Σ(precio aplicable × Inventario Final) de registros con Inventario Final > 0 y precio calculable; USA → priceUSA, CHINA → priceChina.', 'USD', 'Dataset canónico Distribución Tier Inventario', 'Precio ausente nunca se convierte en $0; los agregados suman solo importes calculables. Reutilizado por Executive Summary, Resumen Ejecutivo y Excel para evitar un segundo cálculo.'),
...
export const DEFINITION_GROUPS = Object.freeze({
  executive: Object.freeze(['tierInventorySku', 'tierInventoryUnits', 'tierInventoryValue', 'salesValue', 'replenishmentValue', 'valueShare']),
  ...
  distributions: Object.freeze(['tier', 'tierInventorySku', 'tierInventoryUnits', 'tierInventoryValue', 'tierSalesSku', 'tierSalesUnits', 'tierReplenishmentSku', 'tierReplenishmentUnits', 'unitShare', 'valueShare', 'comparison']),
  ...
  eol: Object.freeze(['eolDefined', 'eolDiscountApplicable', 'eolDiscountMinInventory', 'eolDate', 'eolDays', 'eolBucket', 'eolRecommendation', 'rotationPercentage', 'inventoryValue']),
