...
- Fuentes implementadas: texto local existente y Dataverse `productpricelevels`; `VITE_PRODUCT_SOURCE=local` conserva la fuente efectiva y Product Dataverse no está activado en producción. La ejecución productiva Phase1-046 proporcionada confirmó que el HTTP 400 vigente se origina en el candidato `producturl`, sin demostrar que el campo funcional no exista. Phase1-048/050/052 mantiene una consulta temporal y acotada de metadata para resolver su nombre real; Phase1-052 corrige la resolución de EntityDefinition después del `ENTITY_DEFINITION/FAIL`, sin modificar este selector ni ejecutar otra lectura productiva durante la implementación.
...
- Ruta Dataverse preparada: `GET /api/products/master` -> Product Service -> Product Price Level Gateway -> Dataverse Client -> `productpricelevels`; el frontend no envía OData.
...
| `productUrl` | `producturl` (candidato vigente inválido en `$select`; LogicalName real pendiente de ejecución metadata Phase1-048/050/052) |
| `priceUSA` | `amount` cuando `crbbe_origen = USA` |
| `priceChina` | `amount` cuando `crbbe_origen = CHINA` |

El gateway aplica en `$filter` únicamente `crbbe_companiacompradora = IOCA USA INC` o `SAND SPORTS, CORP.` y repite la allowlist defensivamente en backend. Consolida por SKU; `amount = 0` conserva un precio real igual a cero y `amount null|undefined`, una fila ausente USA o una fila ausente CHINA producen `null` en el precio correspondiente. Valores numéricos distintos del mismo SKU/origen/comprador, o entre ambos compradores sin precedencia autorizada, detienen la carga con `409 / PRODUCT_MASTER_CONFLICT`; cero contra otro número distinto también bloquea, mientras `null`/ausente no compite con un valor real. La misma protección aplica a divergencias no vacías de `productName`, `brand`, `category`, `level`, `status`, `discontinuationDate`, `creationDate`, `imageUrl` o `productUrl`: strings, URLs y etiquetas FormattedValue se comparan con `trim()` y las fechas mediante representación canónica. Vacío más valor no es conflicto; dos valores no vacíos diferentes bloquean sin elegir precedencia. Los nombres vigentes permanecen exclusivamente en Product Price Level Gateway; Phase1-048/050/052 no los corrige y solo agrega lectura interna temporal y observabilidad sanitizada después del fallo específico. La respuesta pública no expone metadata ni detalles internos del conflicto. El contrato Product, Repository, Provider, Application Service, Record Assembler, valorizaciones, exportaciones y presentación conservan `productUrl` y `null` sin convertirlo en cero. `fechaStr` se deriva de la fecha de descontinuación mediante `normalizeFechaStr` en ambas rutas: fecha válida local/ISO/con hora produce `YYYY-MM-DD`, y ausencia o invalidez produce `""`, sin añadir una columna Dataverse ni alterar las fechas de dominio.
...
- Estado Product: **PRODUCT METADATA ENTITY RESOLUTION FIXED / NOT DEPLOYED / NOT EXECUTED / NOT ACTIVATED** por Phase1-052. La evidencia productiva proporcionada confirma `productpricelevels`, los otros doce campos candidatos, el fallo individual de `producturl` y `TRIGGER/REACHED` seguido de `ENTITY_DEFINITION/FAIL`; `VITE_PRODUCT_SOURCE=local` permanece como fuente efectiva y Phase1-052 no ejecutó metadata contra Dataverse productivo.
...
- Smoke Product Phase1-042: `GET /api/products/master` sin parámetros, activado solo mediante `?phase1-042-product-smoke=1`. Requiere cuenta MSAL, adquiere el Bearer delegado por el mecanismo existente y aplica timeout; la respuesta se reduce en memoria a status, cantidad, etapas JWT/Dataverse, diagnóstico y presencia estructural de precios nullable y etiquetas legibles. No se publican SKU, atributos, precios, URLs, token, headers, payload, query strings, PII o secretos. La evidencia productiva proporcionada confirma que, con `productpricelevels`, el request llegó a Dataverse y devolvió `HTTP 400 / DATAVERSE_INVALID_FIELD_OR_FILTER`.
- Diagnóstico seguro Phase1-020: conserva la clasificación sanitaria de fallos, incluido `DATAVERSE_INVALID_FIELD_OR_FILTER`, sin exponer detalles técnicos en el contrato HTTP. Phase1-046 agrega temporalmente probes Product once-per-process que observan solo status y emiten un allowlist técnico `PASS|FAIL`. Phase1-048/050/052 se encadena únicamente al `producturl=FAIL`: emite `TRIGGER/REACHED`; consulta `EntityDefinitions` con `$select=LogicalName,EntitySetName` y `$filter=EntitySetName eq 'productpricelevels'`, sin `$top`; exige una única coincidencia exacta, usa su LogicalName para navegar a `Attributes` y filtra por `url|product|producto`. Distingue `ENTITY_DEFINITION/PASS|FAIL`, `ATTRIBUTES/PASS|FAIL` y `CANDIDATES/FOUND|NONE`; por atributo registra solo `LogicalName`, `SchemaName`, `AttributeType`, `IsValidForRead` y `CANDIDATE|NOT_CANDIDATE`. No lee filas Product, no cambia mapping/contrato y los diagnósticos deben retirarse después de la corrección. Los diagnósticos temporales Customer Phase1-022/024 continúan fuera del runtime.
