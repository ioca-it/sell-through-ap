...
- Fuentes implementadas: texto local existente y Dataverse `productpricelevels`; `VITE_PRODUCT_SOURCE=local` conserva la fuente efectiva y Product Dataverse no está activado en producción. La evidencia productiva proporcionada después de Phase1-044 confirma el Entity Set plural y acota el fallo vigente a `HTTP 400 / DATAVERSE_INVALID_FIELD_OR_FILTER`. Phase1-046 prepara diagnóstico temporal backend sin ejecutar otra lectura productiva ni modificar este selector.
...
- Estado Product: **INVALID QUERY DIAGNOSTIC IMPLEMENTED / NOT DEPLOYED / NOT EXECUTED / NOT ACTIVATED** por Phase1-046. La evidencia productiva proporcionada confirma `productpricelevels` y el nuevo `HTTP 400 / DATAVERSE_INVALID_FIELD_OR_FILTER`; `VITE_PRODUCT_SOURCE=local` permanece como fuente efectiva y ningún probe Phase1-046 se ejecutó contra Dataverse productivo.
...
- Diagnóstico seguro Phase1-020: conserva la clasificación sanitaria de fallos, incluido `DATAVERSE_INVALID_FIELD_OR_FILTER`, sin exponer detalles técnicos en el contrato HTTP. Phase1-046 agrega temporalmente probes Product once-per-process que observan solo status y emiten un allowlist técnico `PASS|FAIL`; no cambia fuente, mapping ni contrato y debe retirarse después de la corrección. Los diagnósticos temporales Customer Phase1-022/024 continúan fuera del runtime.
