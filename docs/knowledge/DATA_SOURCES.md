...
- Fuentes implementadas: texto local existente y Dataverse `productpricelevels`; `VITE_PRODUCT_SOURCE=local` conserva la fuente efectiva y Product Dataverse no está activado en producción. La ejecución productiva Phase1-046 proporcionada confirmó que el HTTP 400 vigente se origina en el candidato `producturl`, sin demostrar que el campo funcional no exista. Phase1-048/050 mantiene una consulta temporal y acotada de metadata para resolver su nombre real y ahora evidencia su entrada o la etapa interna fallida, sin modificar este selector ni ejecutar otra lectura productiva durante la implementación.
...
- Estado Product: **PRODUCT URL METADATA TRIGGER OBSERVABILITY FIXED / NOT DEPLOYED / NOT EXECUTED / NOT ACTIVATED** por Phase1-050. La evidencia productiva proporcionada confirma `productpricelevels`, los otros doce campos candidatos, el fallo individual de `producturl` y cero eventos Phase1-048; `VITE_PRODUCT_SOURCE=local` permanece como fuente efectiva y Phase1-050 no ejecutó metadata contra Dataverse productivo.
...
- Diagnóstico seguro Phase1-020: conserva la clasificación sanitaria de fallos, incluido `DATAVERSE_INVALID_FIELD_OR_FILTER`, sin exponer detalles técnicos en el contrato HTTP. Phase1-046 agrega temporalmente probes Product once-per-process que observan solo status y emiten un allowlist técnico `PASS|FAIL`. Phase1-048/050 se encadena únicamente al `producturl=FAIL`: emite `TRIGGER/REACHED`, resuelve la definición exacta por `EntitySetName`, consulta su navegación `Attributes` con filtro técnico `url|product|producto` y distingue `ENTITY_DEFINITION/FAIL`, `ATTRIBUTES/FAIL` o `CANDIDATES/FOUND|NONE`; por atributo registra solo `LogicalName`, `SchemaName`, `AttributeType`, `IsValidForRead` y `CANDIDATE|NOT_CANDIDATE`. No lee filas Product, no cambia mapping/contrato y los diagnósticos deben retirarse después de la corrección. Los diagnósticos temporales Customer Phase1-022/024 continúan fuera del runtime.
