...
PHASE1-050 queda **PASS — METADATA TRIGGER OBSERVABILITY FIXED / NOT DEPLOYED / NOT EXECUTED / NOT ACTIVATED**. La evidencia productiva proporcionada confirma que Phase1-046 ejecutó `select_field=producturl / FAIL`, pero Render no registró eventos `PHASE1_048_PRODUCT_URL_METADATA`. La causa local exacta era una brecha de observabilidad: Phase1-048 no emitía al entrar y solo registraba después de que `EntityDefinitions` y `Attributes` terminaran; cualquier fallo previo era absorbido por Phase1-046 sin evento Phase1-048.

El hook permanece inmediatamente después de emitir el `FAIL` individual de `producturl` y antes de continuar los probes Phase1-046. Phase1-048 ahora emite primero `TRIGGER/REACHED`; si falla antes de obtener candidatos distingue `ENTITY_DEFINITION/FAIL` y `ATTRIBUTES/FAIL`, y si completa emite `CANDIDATES/FOUND|NONE` con conteo técnico, seguido de los atributos allowlisted vigentes. La bandera module-scope se establece antes del primer `await`, por lo que el trigger y las consultas continúan máximo una vez por proceso y son seguros ante concurrencia. No se corrigió el Product Gateway, `producturl` permanece en el mapping y contrato vigentes, y no se ejecutó metadata contra Dataverse productivo.
...
- Diagnóstico temporal Product Phase1-048/050: inmediatamente después del `select_field=producturl / FAIL` de Phase1-046 emite `TRIGGER/REACHED`, consulta la definición exacta por `EntitySetName=productpricelevels` y sus atributos filtrados por `url|product|producto`, y distingue fallos sanitizados de `ENTITY_DEFINITION` y `ATTRIBUTES` o candidatos encontrados. Registra únicamente etapas/resultados técnicos, conteo y las cinco propiedades allowlisted por atributo; corre máximo una vez por proceso y debe retirarse al resolver/corregir el LogicalName definitivo.
...
Después de revisar Phase1-050, solicitar autorización separada para checkpoint y deploy exclusivo del backend, sin cambiar `VITE_PRODUCT_SOURCE=local`. Tras reiniciar el proceso, ejecutar exactamente una vez el arnés existente `?phase1-042-product-smoke=1` y capturar los eventos JSON `PHASE1_048_PRODUCT_URL_METADATA`: `TRIGGER/REACHED` debe confirmar el enlace y el siguiente evento debe aislar `ENTITY_DEFINITION/FAIL`, `ATTRIBUTES/FAIL` o `CANDIDATES/FOUND|NONE`. Solo si hay candidatos se debe preparar otro prompt para corregir el Gateway y retirar simultáneamente Phase1-046/048/050. La activación normal de Product Dataverse permanece como decisión posterior e independiente.
...
- Phase1-048/050 es diagnóstico Product temporal: se activa inmediatamente tras el `FAIL` individual de `producturl` dentro de Phase1-046, antes de los probes posteriores; emite una entrada sanitizada y distingue fallos de resolución de entidad, consulta de atributos o candidatos obtenidos. Consulta metadata acotada a la entidad asociada con `productpricelevels`, corre máximo una vez por proceso y no expone metadata al frontend. `producturl` no se corrige ni elimina hasta disponer de evidencia real; el diagnóstico debe retirarse con la corrección definitiva.
